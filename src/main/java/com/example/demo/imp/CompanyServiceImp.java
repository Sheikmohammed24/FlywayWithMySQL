...
public class CompanyServiceImp implements CompanyService{
	...
	public ResponseEntity<?> save(CompanyDto dto) {
		Company c = Company.builder().id(dto.getId()).company_name(dto.getCompany_name()).location(dto.getLocation())
				.no_of_emp(dto.getNo_of_emp()).projects(dto.getProjects()).build();
		c = repo.save(c);
	...
	public ResponseEntity<?> update(CompanyDto dto) {
		Company c = Company.builder().id(dto.getId()).company_name(dto.getCompany_name()).location(dto.getLocation()).build();
		c = repo.save(c);
